...
//       <h2 className="text-xl font-semibold mb-4">Personnel en Salle</h2>
// <ul className="list-disc pl-5 mb-6">
//   {renderStaffList('Salle').length > 0 ? renderStaffList('Salle') : <li>Aucun personnel en Salle</li>}
// </ul>

// <h2 className="text-xl font-semibold mb-4">Personnel en Cuisine</h2>
// <ul className="list-disc pl-5 mb-6">
//   {renderStaffList('Cuisine').length > 0 ? renderStaffList('Cuisine') : <li>Aucun personnel en Cuisine</li>}
// </ul>
...
const StaffList = () => {
  ...
  const fetchStaff = async () => {
    try {
      ...
      console.log('Personnel récupéré:', response.data);
      setStaff(response.data);
    } catch (error) {
      console.error('Error fetching staff:', error);
      setError('Erreur lors de la récupération du personnel');
  ...
  const handleAddStaff = async () => {
    try {
      ...
      const staffData = {
        ...newStaff,
        amount: parseFloat(newStaff.amount),
        service: { name: newStaff.service }
      };
      console.log('Données à envoyer:', staffData);
      ...
      setNewStaff({ name: '', surname: '', amount: '', service: '', shift: '' });
      setError('');
      fetchStaff(); // Rafraîchir la liste après l'ajout
    } catch (error) {
      console.error('Failed to add staff', error);
      if (error.response) {
  ...
  const renderStaffList = (serviceName) => {
    return staff
      .filter(s => s.service && (
        (typeof s.service === 'string' && s.service === serviceName) ||
        (typeof s.service === 'object' && s.service.name === serviceName)
      ))
      .map(s => (
        <li key={s._id} className="mb-2">
          {s.name} {s.surname}
